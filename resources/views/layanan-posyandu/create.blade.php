...
@section('content_header')
    <h1>Tambah Layanan Posyandu</h1>
@stop

@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title text-white">Form Tambah Layanan Posyandu</h3>
        </div>
        <form action="{{ route('layanan-posyandu.store') }}" method="POST">
            @csrf
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="number" step="0.1" name="berat" id="berat" class="form-control @error('berat') is-invalid @enderror" 
                                   value="{{ old('berat') }}" placeholder="0.0">
                            @error('berat')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="vitamin" id="vitamin" class="form-control @error('vitamin') is-invalid @enderror" 
                                   value="{{ old('vitamin') }}" placeholder="Jenis vitamin yang diberikan">
                            @error('vitamin')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea name="konseling" id="konseling" rows="4" class="form-control @error('konseling') is-invalid @enderror" 
                                      placeholder="Catatan konseling atau saran yang diberikan">{{ old('konseling') }}</textarea>
                            @error('konseling')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
